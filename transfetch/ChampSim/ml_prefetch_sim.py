...
def run_command():
    ...
    if not args.no_base:
        for name, binary in zip(baseline_names, baseline_binaries):
            ...
            if seed is not None:
                # print("lwj: seed execution_trace")
                # print(execution_trace)
    ...
    if args.prefetch is not None:
        ...
        if seed is not None:
            ...
            cmd = '{binary} -prefetch_warmup_instructions {warm}000000 -simulation_instructions {sim}000000 -seed {seed} -traces {trace} < {prefetch} > {results}/{base_trace}-{base_binary}.txt 2>&1'.format(
                prefetch=args.prefetch, binary=default_prefetcher_binary, warm=args.num_prefetch_warmup_instructions, sim=args.num_instructions,
                trace=execution_trace, seed=seed, results=args.results_dir,
                base_trace=os.path.basename(execution_trace), base_binary=os.path.basename(default_prefetcher_binary))
        else:
            ...
            cmd = '{binary} -prefetch_warmup_instructions {warm}000000 -simulation_instructions {sim}000000 -traces {trace} < {prefetch} > {results}/{base_trace}-{base_binary}.txt 2>&1'.format(
                prefetch=args.prefetch, binary=default_prefetcher_binary, warm=args.num_prefetch_warmup_instructions, sim=args.num_instructions,
                trace=execution_trace, results=args.results_dir, base_trace=os.path.basename(execution_trace),
                base_binary=os.path.basename(default_prefetcher_binary))
        print('Running "' + cmd + '"')
...
def compute_stats(trace, prefetch=None, base=None, baseline_name=None):
    ...
    # print("lwj: pf_data before accessing:", pf_data)
    # if pf_data is None:
    #     print("pf_data is None, skipping this computation.")
    # return  # 或者其他适当的处理
...
def eval_command():
    ...
    for fn in os.listdir(args.results_dir):
        ...
        if 'from_file' in fn:
            ...
            # print("traces_prefetch:")
            # print(traces[trace]['prefetch'])
            # print("traces[trace]:")
            # print(traces[trace])
        else:
    ...
    with open(args.output_file, 'w') as f:
        # print('\n'.join(stats), file=f)
        print('\n'.join(s for s in stats if s is not None), file=f)
